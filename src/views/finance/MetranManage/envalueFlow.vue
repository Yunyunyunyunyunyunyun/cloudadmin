  ...
  <div>
    ...
    <div class="card-container">
      <div class="card-title">
        <a-row>
          <a-col :span="12">
            环保值流水 <span style="font-size:12px;color:#999;margin-left:10px"><a-icon type="info-circle" /> 用户存在跨区域流动交易行为，数据可能存在误差</span>
          </a-col>
...
export default {
  ...
  methods: {
    ...
    getType () {
      this.$httpPost(this.$basePath.environmentType)
        .then(res => {
    ...
    toExport () {
      var param = Object.assign(this.form.getFieldsValue(), this.area)
      param.menuId = '75f205fa654f47ceaee0f3343f49672c'
      if (this.timeArr.length > 0) {
        param.startTime = new Date(this.timeArr[0].format('YYYY/MM/DD') + ' 00:00:00').getTime()
        param.endTime = new Date(this.timeArr[1].format('YYYY/MM/DD') + ' 23:59:59').getTime()
      }
      this.toExportExcel(this.$basePath.envFlowExport, param, '环保值流水.xlsx')
    },
